...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['CLONEFORCE_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['CLONEFORCE_LOG'] or 'warn' if it isn't set.
   */
...
export class Cloneforce {
   ...
   */
  constructor({
    baseURL = readEnv('CLONEFORCE_BASE_URL'),
    apiKey = readEnv('CLONEFORCE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.CloneforceError(
        'The CLONEFORCE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Cloneforce client with an apiKey option, like new Cloneforce({ apiKey: \'My API Key\' }).'
      );
    }
    ...
    this.timeout = options.timeout ?? Cloneforce.DEFAULT_TIMEOUT /* 1 minute */;
    this.logger = options.logger ?? console;
    const defaultLogLevel = 'warn';
    // Set default logLevel early so that we can log a warning in parseLogLevel.
    this.logLevel = defaultLogLevel;
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('CLONEFORCE_LOG'), 'process.env[\'CLONEFORCE_LOG\']', this) ?? defaultLogLevel;
  ...
  static Cloneforce = this;
  static DEFAULT_TIMEOUT = 60000 // 1 minute

  static CloneforceError = Errors.CloneforceError;
...
export declare namespace Cloneforce {
